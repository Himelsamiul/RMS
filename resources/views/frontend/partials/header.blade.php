<div class="navbar navbar-expand-lg bg-light navbar-light sticky-top">
    <div class="container-fluid">
        <a href="#" class="navbar-brand">Olivia</span></a>
        <div class="collapse navbar-collapse justify-content-between" id="navbarCollapse">
            <div class="navbar-nav ml-auto">
                ...
                <li class="nav-item dropdown">
                    <a href="" class="btn btn-secondary" class="nav-link" style="margin-right: 10px;"
                        data-toggle="dropdown">Category<i class="fa fa-angle-down"></i></a>
                    <ul class="dropdown-menu">
                        @foreach ($categories as $data)
                        <li><a href="{{ route('category.menu', $data->id) }}" class="dropdown-item">{{ $data->name
                                }}</a>
                        </li>
                        @endforeach
                    </ul>
                ...
                <a class="btn btn-outline-secondary" style="margin-right: 10px;" href="{{ route('view.cart') }}">Cart({{
                    session()->get('cart') ?
                    count(session()->get('cart')) : 0 }})
                    <span class="navbar navbar-expand-lg bg-light navbar-light">

                    </span>
                </a>
                @auth('customerGuard')
                <i class='fas fa-id-card' style='font-size:24px;'> <a style="margin-right: 10px;"
                        href="{{ route('profile.view') }}">{{
                        auth('customerGuard')->user()->name }}</a></i>
