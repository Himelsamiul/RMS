              ...
              <div class="card-body p-4">
                <div class="row d-flex justify-content-between align-items-center">
                  <div class="col-md-2 col-lg-2 col-xl-2">
                    <img src="{{ url('/app/image/menu/'.$cartItem['image']) }}" class="img-fluid rounded-3" alt="Cotton T-shirt">
                  </div>
                  <div class="col-md-3 col-lg-3 col-xl-3">
                    <p class="lead fw-normal mb-2">{{ $cartItem['name'] }}</p>
                  </div>
                  <div class="col-md-3 col-lg-3 col-xl-2 d-flex">
                    <button data-mdb-button-init data-mdb-ripple-init class="btn btn-link px-2" onclick="updateQuantity({{ $cartItem['id'] }}, -1, {{ $cartItem['price'] }})">
                      <i class="fas fa-minus"></i>
                    </button>
                    <input id="quantity-{{ $cartItem['id'] }}" min="0" name="quantity" value="{{ $cartItem['quantity'] }}" type="number" class="form-control form-control-sm" onchange="updateSubtotal({{ $cartItem['id'] }}, {{ $cartItem['price'] }})" />
                    <button data-mdb-button-init data-mdb-ripple-init class="btn btn-link px-2" onclick="updateQuantity({{ $cartItem['id'] }}, 1, {{ $cartItem['price'] }})">
...
<script>
  function updateQuantity(cartId, change, price) {
    ...
    if (newQuantity >= 0) {
      ...
      // Update the session
      updateCartSession(cartId, newQuantity);
    }
  }
  ...
  function updateCartSession(cartId, newQuantity) {
